...
<script type="text/javascript">
    ...
    $(document).ready(function (e) {
        ...
        $('.bookpro').click(function () {
            var id = $(this).attr('dataid');
            var zsize = $('.acttabsize').attr('data-id');
            ...
            addtocart(id, qty, zsize, topping);
            $("#boxproductall").css({
                opacity: '0',
                visibility: 'hidden',
            });
    ...
    function addtocart(id, $sl, size, topping) {
    $.ajax({
        type: 'post',
        url: "gio-hang.html",
        data: {id: id, sl: $sl, act: 'add', size: size , color: topping},
        //dataType:'json',
...
<div class="col-sp-6">
    <div style="padding: 0px 15px;">
                ...
                <tr>
                    ...
                    <td class="titledetailcon">
                       <div class="noidung_tag">
                           <?php 
                            $sql="select * from table_product where type='size' and hienthi = 1 order by stt asc";
                            $d->query($sql);
                            ...
                            if($pos !== false){
                            ?>
                            <a <?php if($kv == 0){ ?> class="boxsizeall acttabsize boxsize_<?=$vv['id']?>" <?php }else{?> class="boxsizeall boxsize_<?=$vv['id']?>"<?php } ?> data-id="<?=$vv['id']?>" data-sta="0" onclick="callsize(<?=$vv['id']?>)"><?=$vv['ten_'.$lang]?></a>
                            <?php } } ?>
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="titledetailcon">
                        <div class="add-item">
                            ...
                            <input type="text" class="quantity" id="quantity" value="1" readonly style="outline: none;width:20px;height:34px;border:none;padding:0 7px;">
        ...
        <div style="float: left;width: 100%;margin-top: 20px;text-align: center;">
            <a class="bookpro" dataid="<?=$prohome['id']?>">Đặt hàng</a>
        </div>
    </div>
</div>
