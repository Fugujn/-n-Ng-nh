...
function save_gioithieu() {
	...
	$id=$_POST["id"];
    if (empty($_POST))
        transfer("Không nhận được dữ liệu", "index.php?com=time&act=capnhat&type=".$type);
    ...
    if ($d->update($data))
        transfer("Dữ liệu được cập nhật", "index.php?com=time&act=capnhat&type=".$type);
    else
        transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=time&act=capnhat&type=".$type);
}
