...
function get_item() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=user&act=man");
    $item = $d->fetch_array();
}
...
function login() {
    ...
    if ($d->num_rows() == 1) {
        $row = $d->fetch_array();
        if (($row['password'] == md5($password)) && ($row['role'] == 3 || $row['role'] == 4 || $row['role'] == 2)) {
    ...
    transfer("Đang kiểm tra dữ liệu", "index.php?com=user&act=login");
}
