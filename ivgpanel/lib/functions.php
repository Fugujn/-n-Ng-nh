...
// SHOW THE NEW THUMB IMAGE
    if ($func == 'imagejpeg')
        ...
        $func($canvas, $folder . $new_file, floor($quality));
...
function price_sp($p, $dt = '') {
    if ($p == '' || $p == 0) {
        $re = "Liên hệ " . $dt;
    } else {
        $re = number_format($p, '0', ',', '.') . ' đ';
    }
    return $re;
}

function price_sp1($p, $dt = '') {
    if ($p == '' || $p == 0) {
        $re = "Liên hệ " . $dt;
    } else {
        $re = number_format($p, '0', ',', '.');
    }
    return $re;
}
function get_add($id) {
    global $d;
    $d->reset();
    $sql = "select * from #_product_list where type = 'quanhuyen1' and id='" . $id . "'";
    $d->query($sql);
...
function get_district($province) {
    ...
    echo $rs_dis['type'] . ' ' . $rs_dis['name'];
}

function tinhtrang($id) {
    global $d;
    $d->reset();
    $sql = "select * from table_tinhtrang where id = ".$id." ";
    $d->query($sql);
    $status = $d->fetch_array();
    echo $status['trangthai'];
}
...
function transfer1($page = "index.php") {
//    $showtext = $msg;
    $page_transfer = $page;
...
function catchuoi_new($str, $length, $minword = 3) {
    $sub = '';
    $len = 0;
...
function change_language($lang1, $lang2, $url) {

    $tachmang = explode("/", $url);
    for ($i = 0; $i < count($tachmang); $i++) {
        if ($tachmang[$i] == $lang1) {
            $tachmang[$i] = $lang2;
        }
    }
